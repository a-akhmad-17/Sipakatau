...
- **Peningkatan Modul Berita (Optimasi Media & Slider)**:
  ...
  - **Perbaikan Kontras CSS (Bug Fix)**: Memperbaiki kontras teks slider berita di Beranda agar tetap berwarna putih cerah di Light Mode (tidak bertabrakan dengan background gelap slider) serta mewarnai ikon tombol bagikan (Facebook, Twitter, WhatsApp) pada halaman detail berita agar memiliki visibilitas tinggi dan tidak memudar.
...
- **Validasi Dokumen Satu-Per-Satu (Granular Document Verification)**: Menambahkan antarmuka tabel validasi berkas pada modal pelacakan admin ([dashboard.php](file:///F:/Xampp/htdocs/SIPAKATAU/app/Views/admin/dashboard.php)) untuk menyetujui (approve) atau menolak (reject) setiap dokumen persyaratan pendaftaran Ormas dan Rekomendasi Kegiatan secara individual.
- **Progress Bar Dinamis Sesuai Verifikasi**: Mengubah kalkulasi persentase progres agar bertambah secara dinamis pada akun pengguna berdasarkan rasio jumlah berkas yang telah diverifikasi oleh admin.
- **Tampilan Status Dokumen di Sisi Pengguna**: Menampilkan badge status verifikasi berkas (Terverifikasi, Ditolak, Sedang Diperiksa) pada riwayat pendaftaran Ormas ([form_pengajuan.php](file:///F:/Xampp/htdocs/SIPAKATAU/app/Views/user/form_pengajuan.php)) dan riwayat Rekomendasi Kegiatan ([dashboard_rekomendasi.php](file:///F:/Xampp/htdocs/SIPAKATAU/app/Views/user/dashboard_rekomendasi.php)).
- **Migrasi Kolom Progres Rekomendasi**: Menambahkan kolom `progress_percentage` pada tabel `trn_rekomendasi` melalui migrasi `AddProgressPercentageToTrnRekomendasi` untuk melacak status kelayakan dokumen kegiatan secara bertahap.
- **Modal Detail Progres Pengguna**: Menambahkan tombol "Detail" di setiap baris tabel riwayat pendaftaran Ormas ([dashboard.php](file:///F:/Xampp/htdocs/SIPAKATAU/app/Views/user/dashboard.php)) dan Rekomendasi Kegiatan ([dashboard_rekomendasi.php](file:///F:/Xampp/htdocs/SIPAKATAU/app/Views/user/dashboard_rekomendasi.php)) yang membuka modal interaktif detail progres pendaftaran secara langsung tanpa harus memuat ulang halaman. Di dalam modal ini, pengguna dapat melihat ringkasan status berkas, progress timeline alur verifikasi, serta tabel status kelayakan masing-masing dokumen persyaratan.
...
- **Peningkatan API Verifikasi Admin**: Menambahkan route dan controller method `verifyDocument()` pada [Admin.php](file:///F:/Xampp/htdocs/SIPAKATAU/app/Controllers/Admin.php) untuk memodifikasi payload status validasi berkas langsung di dalam format JSON kolom `file_berkas` (Ormas) / `file_proposal` (Rekomendasi) serta menghitung ulang bobot progres.
